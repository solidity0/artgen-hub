...
const TRAITS = {
  background: [
    ...
    { id: 'fog',             weight: 10, fill: '#8a9094', pattern: 'fog',            rarity: 'rare' },
  ...
  body: [
    ...
    { id: 'poncho',     weight: 22, rarity: 'common' },
    { id: 'vest',       weight: 18, rarity: 'uncommon' },
    { id: 'cloak',      weight: 14, rarity: 'rare' },
  ...
  headShape: [
    ...
    { id: 'oval',      weight: 24, rarity: 'common' },
    { id: 'hexagon',   weight: 20, rarity: 'common' },
  ...
  eyeShape: [
    ...
    { id: 'half_lid',      weight: 20, rarity: 'common' },
...
function bodyMarkup(bodyId, colorHex, bgFill) {
  ...
  switch (bodyId) {
    ...
    case 'poncho':
      // Straight-edged draped triangle instead of the curved silhouette
      // every other body shares — deliberately different construction, not
      // a variation on the same curve, so it reads as genuinely distinct
      // from suit/turtleneck/tank_top at a glance.
      return `<rect x="284" y="315" width="32" height="35" fill="#0a0a0a"/><path d="M 260 335 L 340 335 L 415 580 L 185 580 Z" fill="${baseHex}"${strokeAttr} opacity="0.93"/>`;
    case 'vest':
      // Same proven silhouette curve as suit, with an open-front center
      // stripe and two strap marks layered on top — safe geometry, distinct
      // detailing.
      return `<rect x="284" y="315" width="32" height="35" fill="#0a0a0a"/><path d="M 282 335 L 282 360 Q 200 365 195 580 L 405 580 Q 400 365 318 360 L 318 335 Z" fill="${shade(baseHex, -20)}"${strokeAttr}/><path d="M 292 355 L 300 580 L 308 355 Z" fill="${shade(baseHex, 25)}"/><rect x="268" y="378" width="16" height="7" fill="${shade(baseHex, -35)}"/><rect x="316" y="378" width="16" height="7" fill="${shade(baseHex, -35)}"/>`;
    case 'cloak':
      // Same proven curve, widened and given a peaked collar for a more
      // dramatic, flowing silhouette than robe.
      return `<rect x="284" y="315" width="32" height="35" fill="#0a0a0a"/><path d="M 278 330 Q 185 385 178 580 L 422 580 Q 415 385 322 330 Q 300 348 278 330 Z" fill="${baseHex}"${strokeAttr} opacity="0.95"/>`;
    case 'exosuit': {
...
function headMarkup(shapeId, colorHex, rng, bgFill) {
  ...
  switch (shapeId) {
    ...
    case 'oval':
      // Same ry as round (bottom edge stays at cy+ry, no mouthMarkup change
      // needed) but a narrower rx — reads as a distinctly taller/thinner
      // head shape from a simple parameter change, no new geometry needed.
      return `<ellipse cx="${cx}" cy="${cy}" rx="95" ry="${ry}" fill="${baseHex}"${strokeAttr}/>`;
    case 'hexagon': {
      // Bounded within the exact same rx/ry box as round, so the bottom
      // point still lands at cy+ry — an angular silhouette using the same
      // safe footprint instead of a curve.
      const pts = `${cx - rx * 0.6},${cy - ry} ${cx + rx * 0.6},${cy - ry} ${cx + rx},${cy} ${cx + rx * 0.6},${cy + ry} ${cx - rx * 0.6},${cy + ry} ${cx - rx},${cy}`;
      return `<polygon points="${pts}" fill="${baseHex}"${strokeAttr}/>`;
    }
    case 'visor': {
...
function eyesMarkup(shapeId, colorHex, eyeR, glowR, opts) {
  ...
  else if (shapeId === 'half_lid') {
    // sleepy, half-closed flat-topped shape — calm/tired read, distinct
    // silhouette from every rounded or angular shape already in the set.
    body = [cxL, cxR].map(cx => {
      const w = eyeR, h = Math.round(eyeR * 0.55);
      return `<path d="M ${cx - w} ${cy} Q ${cx - w} ${cy - h} ${cx} ${cy - h} Q ${cx + w} ${cy - h} ${cx + w} ${cy} Q ${cx + w * 0.6} ${cy + h * 0.6} ${cx} ${cy + h * 0.6} Q ${cx - w * 0.6} ${cy + h * 0.6} ${cx - w} ${cy} Z" fill="${colorHex}"/>`;
    }).join('');
  }
...
function accessoryMarkup(rng, accId, eyeCtx) {
  ...
  if (accId === 'tears') {
    ...
    } else if (eyeCtx.eyeShape === 'half_lid') {
      bottomOffset = Math.round(eyeR * 0.33); // lower curve's own reach below center
