...
const TRAITS = {
  ...
  body: [
    ...
    { id: 'engine',     weight: 14, rarity: 'rare' },
  ...
  headShape: [
    ...
    { id: 'ghost_head', weight: 20, rarity: 'common' },
...
function bodyMarkup(bodyId, colorHex, bgFill) {
  ...
  switch (bodyId) {
    ...
    case 'engine': {
      // Straight-edged, boxy torso (not a variation on the curved
      // suit/robe silhouette) with a visible mechanical core and pipe
      // details — a machinery-themed body distinct from armored's plated
      // suit and exosuit's jointed-limb look.
      const coreColor = shade(baseHex, 30);
      const darkColor = shade(baseHex, -30);
      return `<rect x="284" y="315" width="32" height="35" fill="#0a0a0a"/><path d="M 270 335 L 330 335 L 340 360 L 400 580 L 200 580 L 260 360 Z" fill="${baseHex}"${strokeAttr}/><circle cx="300" cy="430" r="38" fill="${darkColor}"/><circle cx="300" cy="430" r="28" fill="${coreColor}" opacity="0.6"/><rect x="260" y="500" width="14" height="50" fill="${darkColor}"/><rect x="326" y="500" width="14" height="50" fill="${darkColor}"/>`;
    }
    case 'exosuit': {
...
function headMarkup(shapeId, colorHex, rng, bgFill) {
  ...
  switch (shapeId) {
    ...
    case 'ghost_head': {
      // Same solid ellipse silhouette as round (identical bottom edge,
      // cy+ry — no mouthMarkup change needed), with a scalloped "sheet"
      // fringe bitten into the very bottom using background-colored
      // circles — the same proven gap-through-background technique
      // 'split' already uses, not new/unverified path math.
      const scallops = 4;
      const scallopR = rx / scallops * 0.75;
      let bites = '';
      for (let i = 0; i < scallops; i++) {
        const bx = cx - rx + (i + 0.5) * (rx * 2 / scallops);
        bites += `<circle cx="${bx.toFixed(1)}" cy="${cy + ry}" r="${scallopR.toFixed(1)}" fill="${bgFill}"/>`;
      }
      return `<ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="${baseHex}"${strokeAttr}/>${bites}`;
    }
    case 'visor': {
