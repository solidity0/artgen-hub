...
const TRAITS = {
  background: [
    ...
    { id: 'swamp',           weight: 10, fill: '#33402a', pattern: 'swamp',          rarity: 'rare' },
    { id: 'static',          weight: 10, fill: '#4a4a4a', pattern: 'static',         rarity: 'rare' },
    { id: 'eclipse',         weight: 8,  fill: '#0c0c14', pattern: 'eclipse',        rarity: 'rare' },
    // Rare already had a healthy COUNT of backgrounds (8) — the actual problem
    // was that every single one is gray/near-black. A 1/1 piece forced to
    // rare across every category had nowhere to go but gray, no matter how
    // many gray options existed. These stay dark (matching the collection's
    // tone) but pull from different color families than gray/black, so rare
    // pieces get genuine variety instead of just more of the same shade.
    { id: 'blood_moon',     weight: 10, fill: '#1a0505', pattern: 'blood_moon',     rarity: 'rare' },
    { id: 'toxic_haze',     weight: 10, fill: '#0d1a0a', pattern: 'toxic_haze',     rarity: 'rare' },
    { id: 'deep_ocean',     weight: 10, fill: '#04101f', pattern: 'deep_ocean',     rarity: 'rare' }
  ],
  body: [
  ...
  headColor: [
    ...
    // deep_black being common-only meant 1/1s (and any "Rare" character-type
    // generation) could never actually land on it — headColor already has
    // real rare options, so the tier-fallback never reaches down to common.
    // Same fix as radiant_glow/ninja_black: a rare-tier twin sharing the
    // identical hex and dark-background exclusion, so the color itself
    // stays reachable at rare tier without changing deep_black's own
    // common availability.
    { id: 'obsidian', hex: '#0a0a0a', weight: 10, rarity: 'rare' }
  ],
  eyeShape: [
    ...
    // eyeShape had the thinnest rare ratio of any category (2 of 5) despite
    // being the most visually prominent trait on the whole piece — almost
    // certainly the single biggest driver of 1/1 pieces looking repetitive.
    // Both new shapes are genuinely new topology (not recolors of an
    // existing shape), same dark tone as the rest of the collection.
    ...
    // Same fix as radiant_glow/ninja_black/obsidian: spiral being uncommon
    // meant it could never reach rare-tier generation, since eyeShape
    // already has real rare options. A rare-tier twin sharing the identical
    // rendering keeps the look reachable at rare tier too.
    { id: 'void_spiral',   weight: 8,  rarity: 'rare' },
  ...
  accessory: [
    ...
    { id: 'ash_fall',     weight: 12, rarity: 'rare' }
  ]
};
...
// mouth-only.
// Black headColor + black bodyColor together is reserved for 1/1 pieces —
// this is a genuinely different kind of rule from everything in
// DISALLOWED_COMBOS below: it depends on isOneOfOne status, which that
// system has no concept of (it only ever compares already-known trait
// ids against each other). Handled as a dedicated check in the composer
// instead of forcing the shared reroll system to understand a condition
// none of its other rules need.
const BLACK_HEAD_COLORS = ['deep_black', 'obsidian'];
const BLACK_BODY_COLORS = ['black', 'ninja_black'];
// Same dark-background set already used for deep_black/obsidian's own
// contrast protection — reused here rather than redefined, since the
// same "everything reads as one indistinct black shape" risk applies.
const DARK_BACKGROUNDS_FOR_BLACK_COMBO = ['starfield', 'cemetery_black', 'blood_moon', 'eclipse', 'deep_ocean', 'toxic_haze', 'cave', 'lightning', 'houses_night', 'matte_black_stars'];

// 'static' is excluded from 1/1 pieces specifically — same kind of rule as
// the black-combo check above (depends on isOneOfOne status, not on other
// trait ids), so it gets the same dedicated-check treatment rather than
// living in DISALLOWED_COMBOS.
const ONE_OF_ONE_EXCLUDED_BACKGROUNDS = ['static'];
...
const DISALLOWED_COMBOS = [
  ...
  // obsidian shares deep_black's exact hex, so it needs the identical
  // dark-background exclusion — same invisibility risk, same fix.
  { headColor: 'obsidian', background: 'starfield' },
  { headColor: 'obsidian', background: 'matte_black_stars' },
  { headColor: 'obsidian', background: 'cemetery_black' },
  { headColor: 'obsidian', background: 'blood_moon' },
  { headColor: 'obsidian', background: 'eclipse' },
  { headColor: 'obsidian', background: 'deep_ocean' },
  { headColor: 'obsidian', background: 'toxic_haze' },
  { headColor: 'obsidian', background: 'cave' },
  { headColor: 'obsidian', background: 'lightning' },
  { headColor: 'obsidian', background: 'houses_night' },
...
function headMarkup(shapeId, colorHex, rng, bgFill) {
  ...
  // — 'onyx' (#0a0a0a) used to be an exact match for 'starfield'/
  // 'cemetery_black' (also #0a0a0a), forcing a lightened substitute that
  // looked washed-out rather than the intended dark tone; removed rather
  // than kept correcting for it. Other headColor/background pairs can still
  // land close enough to need the same treatment, so the logic stays.
  // For SEVERE cases (distance < 15) a
  // stroke alone isn't enough: the fill itself stays invisible against the
...
function eyesMarkup(shapeId, colorHex, eyeR, glowR, opts) {
  ...
  if (shapeId === 'round_glow' || shapeId === 'radiant_glow') {
  ...
  else if (shapeId === 'void_stare') {
    // Inverse of round_glow: solid black voids with just a thin colored rim
    // catching the light, instead of a filled glowing eye. Same two-eye
    // layout/positioning as every non-cyclops shape, genuinely different
    // read (absence instead of light).
    body = [cxL, cxR].map(cx => {
      let pulse = '';
      if (animate) {
        const dur = f2(rnd(3, 5));
        const phase = f2(rnd(0, 2));
        pulse = `<animate attributeName="opacity" values="0.5;0.95;0.5" dur="${dur}s" begin="-${phase}s" repeatCount="indefinite"/>`;
      }
      return `<circle cx="${cx}" cy="${cy}" r="${eyeR}" fill="#050505"/>`
           + `<circle cx="${cx}" cy="${cy}" r="${eyeR}" fill="none" stroke="${colorHex}" stroke-width="2.5" opacity="0.8">${pulse}</circle>`;
    }).join('');
  }
  else if (shapeId === 'jagged_slit') {
    // an angular lightning-bolt-shaped slit, distinct topology from every
    // other shape (circles, ellipses, bars, an X, a single dot) —
    // sharp/aggressive rather than round or soft.
    const w = eyeR;
    body = [cxL, cxR].map(cx => {
      let flicker = '';
      if (animate) {
        const dur = f2(rnd(2, 3.5));
        const phase = f2(rnd(0, 2));
        flicker = `<animate attributeName="opacity" values="1;1;0.3;1" keyTimes="0;0.7;0.76;0.82" dur="${dur}s" begin="-${phase}s" repeatCount="indefinite"/>`;
      }
      const d = `M ${cx - w} ${cy + w * 0.3} L ${cx - w * 0.3} ${cy - w * 0.4} L ${cx + w * 0.1} ${cy + w * 0.2} L ${cx + w} ${cy - w * 0.3}`;
      return `<path d="${d}" fill="none" stroke="${colorHex}" stroke-width="${Math.round(eyeR * 0.32)}" stroke-linecap="round" stroke-linejoin="round">${flicker}</path>`;
    }).join('');
  }
  else if (shapeId === 'dot_eyes') {
    // simple small solid dots — minimal, blank stare, deliberately the
    // plainest shape in the whole set (a common-tier anchor, not another
    // elaborate design).
    const r = Math.round(eyeR * 0.35);
    body = [cxL, cxR].map(cx => {
      let blink = '';
      if (animate) {
        const bdur = f2(rnd(4, 7)), bphase = f2(rnd(0, 3));
        blink = `<animateTransform attributeName="transform" type="scale" additive="sum" values="1 1;1 1;1 0.1;1 1;1 1" keyTimes="0;0.92;0.96;1;1" dur="${bdur}s" begin="-${bphase}s" repeatCount="indefinite"/>`;
      }
      return animate
        ? `<g transform="translate(${cx} ${cy})">${blink}<circle cx="0" cy="0" r="${r}" fill="${colorHex}"/></g>`
        : `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${colorHex}"/>`;
    }).join('');
  }
  else if (shapeId === 'spiral' || shapeId === 'void_spiral') {
    ...
    // of primitive circles/rects/lines.
    body = [cxL, cxR].map(cx => {
      let spin = '';
      if (animate) {
        const dur = f2(rnd(3, 6));
        spin = `<animateTransform attributeName="transform" type="rotate" from="0 ${cx} ${cy}" to="360 ${cx} ${cy}" dur="${dur}s" repeatCount="indefinite"/>`;
      }
      let path = `M ${cx} ${cy}`;
      const turns = 2.5, steps = 24;
      for (let i = 1; i <= steps; i++) {
        const t = i / steps, ang = t * turns * Math.PI * 2, r = t * eyeR;
        path += ` L ${(cx + Math.cos(ang) * r).toFixed(1)} ${(cy + Math.sin(ang) * r).toFixed(1)}`;
      }
...
function backgroundOverlay(rng, pattern) {
  ...
  if (pattern === 'cloud') {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      out += `<rect x="0" y="${y.toFixed(1)}" width="600" height="${h.toFixed(1)}" fill="#7fff6b" opacity="${(0.06 + rng() * 0.08).toFixed(2)}"/>`;
    }
    for (let i = 0; i < 12; i++) out += `<circle cx="${Math.round(rng() * 600)}" cy="${Math.round(rng() * 600)}" r="${(1 + rng() * 2).toFixed(1)}" fill="#9fff8a" opacity="${(0.15 + rng() * 0.2).toFixed(2)}"/>`;
  }
  else if (pattern === 'deep_ocean') {
    for (let i = 0; i < 12; i++) {
      const x = Math.round(rng() * 600), y = Math.round(rng() * 600), r = 2 + rng() * 5;
      out += `<circle cx="${x}" cy="${y}" r="${r.toFixed(1)}" fill="none" stroke="#3a7ca8" stroke-width="1" opacity="${(0.2 + rng() * 0.25).toFixed(2)}"/>`;
    }
    for (let i = 0; i < 3; i++) {
      const y = 100 + i * 180 + rng() * 40;
      out += `<path d="M 0 ${y.toFixed(1)} Q 150 ${(y - 15).toFixed(1)} 300 ${y.toFixed(1)} T 600 ${y.toFixed(1)}" fill="none" stroke="#2a5a7f" stroke-width="2" opacity="0.2"/>`;
    }
  }
  return out;
}
...
function accessoryMarkup(rng, accId, eyeCtx) {
  ...
  if (accId === 'ash_fall') {
    // Full-canvas coverage (not clamped to the body-centered box scatter_dots
    // uses) and dark/ember-toned instead of white — falling soot rather than
    // scattered highlight flecks, a genuinely different read even though the
    // underlying construction (scattered small circles) is similar.
    let out = '';
    const n = 10 + Math.floor(rng() * 8);
    for (let i = 0; i < n; i++) {
      const x = Math.round(rng() * 600), y = Math.round(rng() * 600), r = 1 + rng() * 2;
      out += `<circle cx="${x}" cy="${y}" r="${r.toFixed(1)}" fill="#4a3f38" opacity="${(0.35 + rng() * 0.35).toFixed(2)}"/>`;
    }
    return out;
  }
  if (accId === 'scanlines') {
  ...
  if (accId === 'tears') {
    ...
    } else if (eyeCtx.eyeShape === 'void_stare') {
      bottomOffset = eyeR + 3; // solid circle radius plus its rim stroke overhang
    } else if (eyeCtx.eyeShape === 'jagged_slit') {
    ...
    } else if (eyeCtx.eyeShape === 'spiral' || eyeCtx.eyeShape === 'void_spiral') {
...
function generatePiece(index, seed, tier, opts) {
  ...
  let background   = pickByRarity(rng, TRAITS.background, t);
  if (opts && opts.isOneOfOne && ONE_OF_ONE_EXCLUDED_BACKGROUNDS.includes(background.id)) {
    const altPool = TRAITS.background.filter(b => !ONE_OF_ONE_EXCLUDED_BACKGROUNDS.includes(b.id));
    background = pickByRarity(rng, altPool, t);
  }
  ...
  // Black head + black body is reserved for 1/1 pieces specifically —
  // ordinary generatePiece() calls disallow it outright (default, no opt-in
  // needed); the UI passes opts.allowBlackCombo only when actually building
  // a 1/1 piece. Even then, pairing it with a dark background too is
  // avoided regardless — an all-black piece on an all-black background
  // loses all definition, which isn't something being 1/1 should excuse.
  if (BLACK_BODY_COLORS.includes(bodyColor.id) && BLACK_HEAD_COLORS.includes(headColor.id)) {
    const allowed = !!(opts && opts.allowBlackCombo) && !DARK_BACKGROUNDS_FOR_BLACK_COMBO.includes(background.id);
    if (!allowed) {
      const altPool = TRAITS.headColor.filter(c => !BLACK_HEAD_COLORS.includes(c.id));
      headColor = pickByRarity(rng, altPool, t);
    }
  }
...
const api = {
  ...
  BLACK_HEAD_COLORS, BLACK_BODY_COLORS, DARK_BACKGROUNDS_FOR_BLACK_COMBO
};
