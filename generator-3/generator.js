...
function handLettering(word,cx,baseY,color,rng,rot,sc){
  ...
  for(const ch of word){
    ...
    const h=up?15:10, adv=ch.toLowerCase()==='o'?(up?13:10):(up?12:11);
    glyphs.push({ch,up,h,x:w}); w+=adv+2;
  }
  let d='';
  for(const g of glyphs){
    const x=cx-w/2+g.x, y=baseY+j(1.2);
    if(g.ch.toLowerCase()==='o'){
      const rx=g.up?5.6:4.4, ry=g.up?7.4:4.9, ox=x+rx+0.5, oy=y-ry;
      d+=`M${(ox+rx*0.2).toFixed(1)},${(oy-ry).toFixed(1)} C${(ox+rx*1.25+j(1)).toFixed(1)},${(oy-ry+j(1)).toFixed(1)} ${(ox+rx*1.2+j(1)).toFixed(1)},${(oy+ry+j(1)).toFixed(1)} ${(ox).toFixed(1)},${(oy+ry).toFixed(1)} C${(ox-rx*1.3+j(1)).toFixed(1)},${(oy+ry+j(1)).toFixed(1)} ${(ox-rx*1.2+j(1)).toFixed(1)},${(oy-ry*1.1+j(1)).toFixed(1)} ${(ox+rx*0.45+j(1)).toFixed(1)},${(oy-ry*0.95).toFixed(1)} `;
    } else if(g.up){ // D
      const top=y-g.h;
      d+=`M${(x+1+j(0.8)).toFixed(1)},${(top).toFixed(1)} L${(x+1.4+j(0.8)).toFixed(1)},${(y).toFixed(1)} `;
      d+=`M${(x).toFixed(1)},${(top+0.5).toFixed(1)} C${(x+11+j(1.5)).toFixed(1)},${(top-1+j(1)).toFixed(1)} ${(x+12+j(1.5)).toFixed(1)},${(y+1+j(1)).toFixed(1)} ${(x+0.5).toFixed(1)},${(y-0.3).toFixed(1)} `;
    } else { // d
      const rx=4.2, ry=4.8, ox=x+rx, oy=y-ry;
      d+=`M${(ox+rx).toFixed(1)},${(oy-ry*0.6).toFixed(1)} C${(ox+rx*0.2).toFixed(1)},${(oy-ry*1.3+j(1)).toFixed(1)} ${(ox-rx*1.4+j(1)).toFixed(1)},${(oy-ry*0.4).toFixed(1)} ${(ox-rx*0.6).toFixed(1)},${(oy+ry*0.9).toFixed(1)} C${(ox+rx*0.3).toFixed(1)},${(oy+ry*1.5+j(1)).toFixed(1)} ${(ox+rx*1.1).toFixed(1)},${(oy+ry*0.4).toFixed(1)} ${(ox+rx).toFixed(1)},${(oy).toFixed(1)} `;
...
function accessoryMarkup(style,cx,cy,eyeL,eyeR,eyeY,eyeR2,ink,eyeHex,rng,headRy,hairId,isOneOfOne,bgHex,clothingId) {
  ...
  if(style==='glasses'){
    ...
    if(TOP_SAFE_HAIR.includes(hairId)){
      ...
      // right where the tag used to land — push it lower to clear that row.
...
function renderFromTraits(picks, index, seed, opts) {
  ...
  // outline/light_shade/hatch_shade/heavy_shade/splatter are ALL fundamentally
  // transparent heads — only ink_black has genuine opaque fill. So the actual
  // backdrop behind the eyes, in most cases, is just the piece's own page
  // background showing straight through. If eyeColor's brightness is close to
  // that backdrop's brightness (dark eyes on a dark background, light eyes on
  // a light one), the eye can vanish outright — not merely get lost in clutter.
  // The clearing disc color must contrast with the EYE itself (light backing
  // for dark eyes, dark backing for light eyes) — using the background's own
  // color here, as the previous version did, only helped with visual clutter
  // and did nothing for this more fundamental case, since a same-color disc
  // can't create contrast against itself.
  const eyeLuma = hexLuma(eyeColor.hex);
  const effectiveBackdropLuma = headFill.id === 'ink_black' ? hexLuma('#0a0a0a') : hexLuma(background.bg);
  const needsClearing = Math.abs(eyeLuma - effectiveBackdropLuma) < 70;
  let eyeClearing = '';
  if (needsClearing) {
    const clearingColor = eyeLuma < 128 ? '#e8e8e8' : '#0a0a0a';
    const clearR = eyeR * 1.55;
    eyeClearing =
      `<ellipse cx="${eyeL}" cy="${eyeY}" rx="${clearR.toFixed(1)}" ry="${(clearR*0.95).toFixed(1)}" fill="${clearingColor}" opacity="0.4"/>` +
      `<ellipse cx="${eyeR2_pos}" cy="${eyeY}" rx="${clearR.toFixed(1)}" ry="${(clearR*0.95).toFixed(1)}" fill="${clearingColor}" opacity="0.4"/>`;
  }
  svg+=eyeClearing;
...
// 75% dark (spread across every dark option), 25% grey / paper
const ONE_OF_ONE_BACKGROUND_WEIGHTS = [
  { id: 'matte_black_stars',     weight: 11 },
  { id: 'matte_black_red_stars', weight: 10  },
  { id: 'matte_black_wash',      weight: 10  },
  { id: 'matte_black_splat',     weight: 10  },
  { id: 'matte_black',           weight: 9  },
  { id: 'ink_wash',              weight: 9  },
  { id: 'black_splat',           weight: 9  },
  { id: 'black_clean',           weight: 7  },
  { id: 'white_dots',            weight: 3  },
  { id: 'grey_wash',             weight: 4  },
  { id: 'white_clean',           weight: 4  },
  { id: 'light_hatch',           weight: 4  },
  { id: 'cross_hatch',           weight: 3  },
